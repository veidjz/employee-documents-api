...
describe('Submissions concurrency (e2e)', () => {
  ...
  it('keeps a single active version when three resubmissions race', async () => {
    const requirementId = await linkRequirement()

    await Promise.all(
      ['first.pdf', 'second.pdf', 'third.pdf'].map((fileName) =>
        request(app.getHttpServer())
          .post(`/requirements/${requirementId}/submissions`)
          .send({ fileName, contentType: 'application/pdf', sizeBytes: 184320 })
          .expect(201),
      ),
    )
    ...
    expect(stored.map((submission) => submission.version)).toEqual([1, 2, 3])
    expect(stored.filter((submission) => submission.isActive)).toHaveLength(1)
    expect(stored[2].isActive).toBe(true)
    expect(requirement.currentVersion).toBe(3)
  })
})
